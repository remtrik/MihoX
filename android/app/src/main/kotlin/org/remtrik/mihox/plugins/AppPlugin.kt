...
class AppPlugin : FlutterPlugin, MethodChannel.MethodCallHandler, ActivityAware {
    ...
    private fun initShortcuts(label: String) {
        val shortcut = ShortcutInfoCompat.Builder(MihoXApplication.getAppContext(), "toggle")
            .setShortLabel(label)
            .setIcon(
            ...
            .setIntent(MihoXApplication.getAppContext().getActionIntent("CHANGE"))
            .build()
    ...
    private suspend fun getChinaPackageNames(): String {
        return withContext(Dispatchers.Default) {
            val names = getPackages()
                .map { it.packageName }
                .map { pkg ->
                    async {
                        pkg to chinaPackageCache.getOrPut(pkg) { isChinaPackage(pkg) }
                    }
                }
                .awaitAll()
                .filter { it.second }
    ...
    private fun isChinaPackage(packageName: String): Boolean {
        ...
        try {
            ...
            packageInfo.applicationInfo?.publicSourceDir?.let { path ->
                ZipFile(File(path)).use { zip ->
                    val entries = zip.entries().toList()
                    if (entries.any { it.name.startsWith("firebase-") }) return false
                    ...
                    for (packageEntry in entries) {
                        ...
                        val dexFile = try {
                            zip.getInputStream(packageEntry).buffered().use {
