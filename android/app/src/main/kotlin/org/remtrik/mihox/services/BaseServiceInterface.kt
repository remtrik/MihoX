...
import org.remtrik.mihox.MainActivity
import org.remtrik.mihox.R
import org.remtrik.mihox.extensions.getActionPendingIntent
...
fun Service.createMihoXNotificationBuilder(): Deferred<NotificationCompat.Builder> =
    CoroutineScope(Dispatchers.Main).async {
        val stopText = GlobalState.getText("stop")
        val reconnectText = GlobalState.getText("reconnect")
        val intent = Intent(this@createMihoXNotificationBuilder, MainActivity::class.java)

        val pendingIntent = if (Build.VERSION.SDK_INT >= 31) {
            PendingIntent.getActivity(
                this@createMihoXNotificationBuilder,
                0,
                intent,
            ...
            PendingIntent.getActivity(
                this@createMihoXNotificationBuilder, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT
            )
        }

        with(
            NotificationCompat.Builder(
                this@createMihoXNotificationBuilder, GlobalState.NOTIFICATION_CHANNEL
            )
        ) {
            setSmallIcon(R.drawable.ic)
            setContentTitle("MihoX")
            ...
            addAction(
                0, reconnectText, getActionPendingIntent("RECONNECT")
            )
            setShowWhen(false)
