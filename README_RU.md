...
> Примечание: `closeconnections` в самом приложении по умолчанию включён, но при использовании `mihox-settings` состояние задаётся явно — если не передать токен, опция будет выключена.
...
<details>
<summary><strong>mihox-newboard</strong></summary>
...
<details>
<summary><strong>mihox-newdomain</strong></summary>
...
<details>
<summary><strong>mihox-buyplan</strong></summary>

Прямая ссылка на оплату/продление подписки. Кнопка «Продлить подписку» появляется под карточкой трафика на новой главной (`mihox-newboard`) только когда до конца подписки остаётся меньше 3 дней (в т.ч. если подписка уже истекла). По нажатию открывается переданная ссылка.
...
<details>
<summary><strong>mihox-buytraffic</strong></summary>

Прямая ссылка на докупку трафика. Кнопка «Докупить трафик» появляется под карточкой трафика на новой главной (`mihox-newboard`) только когда остаётся меньше 10% от лимита трафика. Если сработали оба триггера (`mihox-buyplan` и `mihox-buytraffic`), кнопки показываются в один ряд.
