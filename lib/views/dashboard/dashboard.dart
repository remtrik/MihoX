...
class _DashboardViewState extends ConsumerState<DashboardView> with PageMixin {
  ...
  @override
  void initState() {
    ref.listenManual(isCurrentPageProvider(PageLabel.dashboard), (prev, next) {
        ...
        final headers =
            ref.watch(
              currentProfileProvider.select(
                (profile) => profile?.providerHeaders,
              ),
            ) ??
            {};
  ...
  void _handleSave() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final dashboardWidgets = children
          .map(DashboardWidget.getDashboardWidget)
          .toList();
      ref
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _addedWidgetsNotifier.value = DashboardWidget.values
          .where((item) => !children.contains(item.widget) && isAllowed(item))
          .map((item) => item.widget)
          .toList();
    });
